...
{{
  ...
  "danger_alert": true/false (위험/긴급 상황 판단 시 true)
    ...
    def legal_list():
        ...
        if role in ('admin','leader'):
            ...
            posts = LegalPost.query.filter(
                (LegalPost.user_id == uid) | (LegalPost.user_id.is_(None))
            ).order_by(LegalPost.created_at.desc()).all()
        else:
    ...
    def legal_appointment_book():
        ...
        title = request.form.get('title', '법률상담')
        # 이틀 후부터 예약 가능
    ...
    @app.route('/legal/issues/ai-suggest', methods=['POST'])
    ...
    @app.route('/legal/issues/toggle/<int:post_id>', methods=['POST'])
    ...
    def psycho_list():
        ...
        if role in ('admin','leader'):
            ...
            posts = PsychoPost.query.filter(
                (PsychoPost.user_id == uid) | (PsychoPost.user_id.is_(None))
    ...
    @app.route('/psycho/post/<int:post_id>/edit', methods=['GET','POST'])
